...
/*//////////////////////////////////////////////
	...
	- i originally tried to keep each sorted word as an array and used the == operation, but it wasn't doing what i expected
	- instead of possibly debugging i just imploded each sorted array back into a string and used ==
	- in retrospect, maybe the name of the flag that checks whether there's been a repetition should be called something different. i was wondering breaking twice out of the nested for, but didn't like the feeling of using break 2; so flag it was

/*//////////////////////////////////////////////

$passphrase_list = "bdwdjjo avricm cjbmj ran lmfsom ivsof
...
$passphrase_list_lines = explode("\n", $passphrase_list);
// $passphrase_list_lines = array_slice($passphrase_list_lines, 0, 3);
$valid_passphrases = 0;

foreach ($passphrase_list_lines as $passphrase_list_line) {
	$cleaned_passphrase_list_line = preg_replace("/[^a-zA-Z ]/", "", $passphrase_list_line);

	$passphrase_words = explode(" ", $cleaned_passphrase_list_line);
	$sorted_passphrase_words = array();

	foreach ($passphrase_words as $passphrase_word) {
		$passphrase_word_array = str_split($passphrase_word);
		asort($passphrase_word_array);
		$sorted_passphrase_words[] = implode($passphrase_word_array);
		// print_r($sorted_passphrase_words);
	}
	print_r($sorted_passphrase_words);

	$repeated_passphrase = false;
	for ($i=0; $i < count($sorted_passphrase_words); $i++) { 
		for ($j=($i+1); $j < count($sorted_passphrase_words); $j++) { 
	// 		// echo "Here are the two arrays I want to test\n";
	// 		// print_r($sorted_passphrase_words[$i]);
	// 		// print_r($sorted_passphrase_words[$j]);
			if ($sorted_passphrase_words[$i] == $sorted_passphrase_words[$j]) {
				echo "I found two that are the same!";
				$repeated_passphrase = true;
			}
		}
	}

	if (!$repeated_passphrase) {
		$valid_passphrases++;
	}

	// print_r($sorted_passphrase_words); exit;
}
// echo count($passphrase_list_lines);
