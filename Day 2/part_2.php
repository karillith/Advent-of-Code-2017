...
/*//////////////////////////////////////////////
	...
	- haha the first iteration i forgot to actually plug $value_1 and $value_2 into the array so I was just comparing the counters my b
...
// print_r($spreadsheet_lines); exit;
$checksum = 0;
...
foreach ($spreadsheet_lines as $spreadsheet_line) {
	...
	for ($i=0; $i < count($values); $i++) { 
		for ($j=0; $j < count($values); $j++) { 
			...
			preg_match("/\d+/", $values[$i], $matches);
			$value_1 = $matches[0];
			preg_match("/\d+/", $values[$j], $matches);
			$value_2 = $matches[0];
			// echo "$value_1 divided by $value_2 equals " . ($value_1 / $value_2) . "\n";
			if ($value_1 % $value_2 == 0) {
				// echo "$value_1 and $value_2 divide evenly!\n";
				$result = $value_1/$value_2;
				// echo "their result is $result\n";
				$checksum += $result;
			}

			// echo "i is {$values[$i]} and j is {$values[$j]}\n";
			// if (trim($values[$i]) % trim($values[$j]) == 0) {
			// 	echo "{$values[$i]} and {$values[$j]} divide evenly!\n";
			// 	$result = trim($values[$i])/trim($values[$j]);
			// 	echo "their result is $result\n";
			// 	$checksum += $result;
			// }
		}
