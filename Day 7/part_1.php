...
/*//////////////////////////////////////////////
	...
	- tfw you realize that you can probably do part 1 without building the structure??? :')
...
$input_lines = explode("\n", $input);
// $mini_trees = array();
$mini_tree_heads = array();
$mini_tree_values = array();
...
foreach ($input_lines as $input_line) {
	...
	if (count($matches[0]) == 1) {
		...
		// $mini_trees[$head] = $matches[0];
		$mini_tree_heads[] = $head;
		foreach ($matches[0] as $match_value) {
			$mini_tree_values[] = $match_value;
		}
	}
}

print_r($mini_tree_heads);
print_r($mini_tree_values);

foreach ($mini_tree_heads as $head) {
$head_exists_elsewhere_flag = false;
	foreach ($mini_tree_values as $value) {
		if ($head == $value) {
			// echo "$head is head and $value is value\n";
			$head_exists_elsewhere_flag = true;
		}
	}
	// echo "flag is set to $head_exists_elsewhere_flag\n";
	// var_dump($head_exists_elsewhere_flag);
	if (!$head_exists_elsewhere_flag) {
		echo "I got it! the root of the structure is $head\n";
	}
}
