...
/*//////////////////////////////////////////////
	...
	- this was tuff ;; everything was bad and recursion wah
	- actually it wasn't as bad as it could be
	- this script only yells at you with the weights of the uneven tree--you have to be the one to calculate what the correct weight needs to be
	- i want to figure it out really but i think i'll move on to the next days before this one exercise burns me out lol
...
foreach ($input_lines as $input_line) {
	preg_match_all("/[a-zA-Z]+/", $input_line, $matches);
	preg_match("/\d+/", $input_line, $match_dig);
	// print_r($matches); exit;
	if (count($matches[0]) == 1) {
		$mini_trees[$matches[0][0]]['tree'] = array();
		$mini_trees[$matches[0][0]]['weight'] = $match_dig[0];
		$mini_trees[$matches[0][0]]['total_weight'] = $match_dig[0];
	} else {
		$head = array_shift($matches[0]);
		$mini_trees[$head]['tree'] = $matches[0];
		$mini_trees[$head]['weight'] = $match_dig[0];
		$mini_trees[$head]['total_weight'] = '';
	}
}

// print_r($mini_trees);
// echo get_total_weight($mini_trees['padx'],  $mini_trees);

$mini_tree_total_weights = array();

foreach ($mini_trees as $mini_tree_name => $mini_tree_values) {
	$mini_tree_total_weights[$mini_tree_name] = get_total_weight($mini_tree_values, $mini_trees);
}
print_r($mini_tree_total_weights);

//calculate total weight


function get_total_weight($subtree, $mini_trees) {
	if ($subtree['total_weight'] != '') {
		return $subtree['total_weight'];
	}

	// $subtotal_weight = $subtree['weight'];
	$weights = array();
	foreach ($subtree['tree'] as $smolertree) {
		$weights[] = get_total_weight($mini_trees[$smolertree], $mini_trees);
		// $subtotal_weight += get_total_weight($mini_trees[$smolertree], $mini_trees);
	}
	...
	foreach ($weights as $weight) {
		if ($weight != $first_weight) {
			print_r($subtree);
			print_r($weights);
			exit("AUGHHH i'm broken");
		}
	}

	return array_sum($weights) + $subtree['weight'];
}
